...
const char * const BITCOIN_CONF_FILENAME = "enox.conf";
const char * const BITCOIN_PID_FILENAME = "enoxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "enox" is a composite category enabling all Enox-related debug output
            if(ptrCategory->count(string("enox"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EnoxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EnoxCore
    // Mac: ~/Library/Application Support/EnoxCore
    // Unix: ~/.enoxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EnoxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EnoxCore";
#else
    // Unix
    return pathRet / ".enoxcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty enox.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override enox.conf
