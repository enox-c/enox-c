...
*Setup instructions for a gitian build of Enox using a Debian VM or physical system.*
...
Clone the git repositories for enox and gitian and then checkout the enox version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the enox repository
...
To build Enox (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the enox repository.
...
```bash
    ...
    From https://github.com/eastcoastcrypto/Enox
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Enox.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/enox-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/enox-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/enox-build.assert
```
